...
import com.imsupra.androidclient.service.IXMPPChatService;
...
public class XMPPService extends GenericService {
	...
	private static final String RECONNECT_ALARM = "com.imsupra.androidclient.RECONNECT_ALARM";
	...
	public void onCreate() {
		...
		IMSupraBroadcastReceiver.initNetworkStatus(getApplicationContext());
		...
		if (intent != null) {
			...
			if (reconnect) {
				...
				if (mConnectionDemanded.get())
					...
					stopSelf(); // started by IMSupraBroadcastReceiver, no connection initiation
				return START_STICKY;
			} else
			if (ping) {
				if (mConnectionDemanded.get()) {
		...
		mConnectingThread = new Thread() {
			...
			public void run() {
				try {
					...
					logError("IMSupraXMPPException in doConnect():");
					e.printStackTrace();
				} finally {
